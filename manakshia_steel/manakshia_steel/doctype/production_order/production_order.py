...
class ProductionOrder(StockController):
	...
	def get_sl_entries(self):
		if not self.source_warehouse or not self.target_warehouse:
			frappe.throw(_("Source and Target Warehouse are both required to process material transfer."))
	...
	def get_custom_gl_entries(self):
		...
		for item in self.get("items"):
			...
			source_account = self.get_inventory_account_dict(
				frappe._dict({"item_code": item.item_code, "warehouse": self.source_warehouse}),
				inventory_account_map,
				warehouse_field="warehouse"
			).get("account")

			target_account = self.get_inventory_account_dict(
				frappe._dict({"item_code": item.item_code, "warehouse": self.target_warehouse}),
				inventory_account_map,
				warehouse_field="warehouse"
			).get("account")

			if not source_account:
				frappe.throw(_("Inventory account not found for source warehouse {0}").format(self.source_warehouse))
			if not target_account:
				frappe.throw(_("Inventory account not found for target warehouse {0}").format(self.target_warehouse))

			amount = flt(item.qty) * flt(
				item.get("rate") or frappe.db.get_value("Item", item.item_code, "valuation_rate") or 0.0
			)
			...
			if amount and source_account != target_account:
				...
				gl_entries.append(
					self.get_gl_dict({
						...
						"remarks": self.get("remarks") or _("Accounting Entry for Production Order Transfer"),
						"debit": amount,
