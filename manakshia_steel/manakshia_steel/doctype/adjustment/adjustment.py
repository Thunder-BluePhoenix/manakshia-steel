    ...
    def validate_items(self):
        for item in self.get("items"):
            ...
            if not item.warehouse:
                frappe.throw(
                    _("Warehouse is mandatory for Item {0} in row {1}").format(
            ...
            if not item.type:
                frappe.throw(
                    _(
                        "Type (Receive/Issue) is mandatory for Item {0} in row {1}"
            ...
            if flt(item.qty) <= 0:
                frappe.throw(
                    _(
                        "Quantity must be greater than zero for Item {0} in row {1}"
                    ).format(item.item_code, item.idx)
                )
