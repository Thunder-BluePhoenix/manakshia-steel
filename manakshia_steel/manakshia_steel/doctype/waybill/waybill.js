frappe.ui.form.on("Waybill", {
  customer_name: function (frm) {
    if (frm.doc.customer_name) {
      frappe.db
        .get_value("Customer", frm.doc.customer_name, [
        ...
        .then((r) => {
          if (r && r.message) {
            let d = r.message;
            let address_parts = [];
            if (d.custom_address_line_1)
              address_parts.push(d.custom_address_line_1);
            if (d.custom_address_line_2)
              address_parts.push(d.custom_address_line_2);

            let city_state = [];
            if (d.custom_citytown) city_state.push(d.custom_citytown);
            if (d.custom_state__province)
              city_state.push(d.custom_state__province);
            if (city_state.length > 0)
              address_parts.push(city_state.join(", "));

            let curr_country = [];
            if (d.custom_country) curr_country.push(d.custom_country);
            if (d.custom_postal_code) curr_country.push(d.custom_postal_code);
            if (curr_country.length > 0)
              address_parts.push(curr_country.join(" - "));
