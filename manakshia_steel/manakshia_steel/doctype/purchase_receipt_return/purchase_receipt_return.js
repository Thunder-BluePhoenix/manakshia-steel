  ...
  refresh(frm) {
    ...
    if (frm.doc.docstatus === 0) {
      frm.add_custom_button(
        __("Fetch Items from Purchase Receipt"),
        function () {
          if (!frm.doc.purchase_receipt) {
            frappe.msgprint({
              title: __("Missing Purchase Receipt"),
              message: __(
                "Please select a <b>Purchase Receipt</b> before fetching items."
              ),
              indicator: "orange",
            });
            return;
          ...
          frappe.call({
            ...
            callback: function (r) {
              if (!r.message) {
                frappe.msgprint(
                  __("Could not load the selected Purchase Receipt.")
                );
                return;
              }
              ...
              frm.set_value(
                "custom_purchase_receipt_type",
                pr.custom_purchase_receipt_type
              );
              frm.set_value(
              ...
              (pr.items || []).forEach(function (item) {
                ...
                row.base_net_rate =
                  item.base_net_rate ||
                  (item.net_rate || item.rate || 0) * conv;
                row.base_net_amount =
                  item.base_net_amount ||
                  (item.net_amount || item.amount || 0) * conv;
                row.price_list_rate = item.price_list_rate || item.rate || 0;
                row.base_price_list_rate =
                  item.base_price_list_rate ||
                  (item.price_list_rate || item.rate || 0) * conv;
                ...
                row.custom_country_of_origin_of_goods =
                  item.custom_country_of_origin_of_goods || null;
                row.custom_country_of_final_destination =
                  item.custom_country_of_final_destination || null;
