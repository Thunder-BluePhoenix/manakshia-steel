...
frappe.ui.form.on("Stock Entry", {
  onload: function (frm) {
    // ── Auto-set stock_entry_type ────────────────────────────────
    // Covers two scenarios:
    // 1. Same-tab SPA navigation: workspace.js intercepts pushState and stores the type in sessionStorage.
    // 2. New-tab / full page load: Frappe's router rewrites the URL to new-stock-entry-xxx before onload fires,
    //    but the browser's Performance Navigation API preserves the initial URL requested from the server.
    if (frm.is_new() && !frm.doc.stock_entry_type) {
      var pending = sessionStorage.getItem("__pending_se_type");
      if (pending) {
        sessionStorage.removeItem("__pending_se_type");
        frm.set_value("stock_entry_type", pending);
      } else {
        try {
          var navs = performance.getEntriesByType("navigation");
          if (navs && navs.length > 0) {
            var origUrl = navs[0].name || "";
            var type_map = {
              "new-issue": "Material Issue",
              "new-receipt": "Material Receipt",
              "new-transfer-in": "Stock Transfer In",
              "new-transfer-out": "Stock Transfer Out",
            };
            for (var key in type_map) {
              if (origUrl.indexOf(key) !== -1) {
                frm.set_value("stock_entry_type", type_map[key]);
                break;
              }
            }
          }
          ...
          console.warn(
            "Performance Navigation API not supported or failed:",
            err
          );
        }
      }
    ...
    // Hide stock_entry_type field for these operational types
    var hide_types = [
      "Material Issue",
      "Material Receipt",
      "Stock Transfer In",
          ...
          callback: function (res) {
            ...
            Promise.all(promises).then(() => {
              issue.items.forEach((row) => {
                let key = row.item_code + "::" + (row.batch_no || "");
                let remaining =
                  (issued_qty[key] || 0) - (received_qty[key] || 0);
...
function toggle_custom_fields(frm) {
  let is_transfer = ["Stock Transfer In", "Stock Transfer Out"].includes(
...
function update_stock_entry_header(frm) {
  ...
  const isTransferEntry =
    frm.doc.stock_entry_type &&
    transferTypes.includes(frm.doc.stock_entry_type);
