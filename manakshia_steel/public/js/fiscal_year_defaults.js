...
(function () {
  ...
  function apply_date_silently(frm, field, effective_date) {
    ...
    if (
      effective_date !== today &&
      Object.prototype.hasOwnProperty.call(frm.doc, "set_posting_time")
    ) {
    ...
    get_fy_defaults(function (fy) {
      ...
      if (date_val < fy.year_start_date || date_val > fy.year_end_date) {
        ...
        frappe.msgprint({
          ...
          indicator: "red",
          message: __(
            "The <b>{0}</b> ({1}) does not belong to your currently active Fiscal Year ({2} to {3}).",
            [
              field_label,
    ...
    if (
      ...
      if (from_wh && from_wh !== warehouse) {
        frappe.msgprint({
          ...
          indicator: "red",
          message: __(
            "The <b>Source Warehouse</b> ({0}) must be your active Unit ({1}).",
            [from_wh, warehouse]
          ),
        });
        frappe.validated = false;
        ...
        frappe.msgprint({
          ...
          indicator: "red",
          message: __(
            "For incoming stock, the <b>Target Warehouse</b> ({0}) must be your active Unit ({1}).",
            [to_wh, warehouse]
          ),
        });
        frappe.validated = false;
      ...
      if (wh_val !== warehouse) {
        ...
        frappe.msgprint({
          ...
          indicator: "red",
          message: __(
            "The <b>{0}</b> ({1}) does not match your currently active Unit ({2}). Please change it or switch units.",
  ...
  POSTING_DATE_DOCTYPES.forEach(function (doctype) {
    ...
    if (doctype === "Stock Entry") {
      events.from_warehouse = function (frm) {
        validate_warehouse(frm, "from_warehouse");
      };
      events.to_warehouse = function (frm) {
        validate_warehouse(frm, "to_warehouse");
      };
    } else {
      events.set_warehouse = function (frm) {
        validate_warehouse(frm, "set_warehouse");
      };
    }
  ...
  const DATE_DOCTYPES = ["Waybill", "Waybill Return"];

  DATE_DOCTYPES.forEach(function (doctype) {
    let events = {
      refresh: function (frm) {
        apply_posting_date(frm, "date");
      },
      date: function (frm) {
...
$(document).ready(function () {
  setInterval(function () {
    ...
    const unit =
      ...
      frappe.defaults.get_default("warehouse");
    const fy =
    ...
    if (document.querySelector("#custom-unit-scope-badge")) {
      let badgeText = document.querySelector(
        "#custom-unit-scope-badge .unit-text"
      );
