frappe.ui.form.on('Customer', {
    after_save(frm) {
        if (!frm.doc.custom_address_line_1 || !frm.doc.custom_citytown || !frm.doc.custom_country) return;
        ...
        frappe.msgprint = function() {}; // disable messages
        ...
        if (!frm.doc.custom_customer_address) {
            frappe.call({
                ...
                callback: function(r) {
                    ...
                    if (!r.exc) {
                        frm.set_value("custom_customer_address", r.message.name);
                        frm.save();
                    }
                }
            });
