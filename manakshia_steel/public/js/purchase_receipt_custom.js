...
function pr_inject_series(frm) {
  ...
  const existing = (field.df.options || "")
    .split("\n")
    .map((s) => s.trim())
    .filter(Boolean);
  const toAdd = Object.values(PR_TYPE_TO_SERIES).filter(
...
function pr_apply_series(frm) {
  ...
  const series =
    PR_TYPE_TO_SERIES[frm.doc.custom_purchase_receipt_type] ||
    PR_DEFAULT_SERIES;
...
frappe.ui.form.on("Purchase Receipt Item", {
  ...
  rejected_warehouse(frm, cdt, cdn) {
    ...
    if (
      ...
      frappe.msgprint(
        __("Accepted and Rejected Warehouse cannot be the same.")
      );
      frappe.model.set_value(cdt, cdn, "rejected_warehouse", "");
    }
  },
...
frappe.ui.form.on("Purchase Receipt", {
  ...
  custom_purchase_receipt_type(frm) {
    ...
    frm.set_value("naming_series", series);
    frappe.show_alert({
      message: __("Naming series set to: {0}", [series]),
      indicator: "blue",
    });
  },
});
