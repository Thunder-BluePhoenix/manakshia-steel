...
frappe.ui.form.on('Material Request', {
    onload: function (frm) {
        if (frm.is_new()) {
            frappe.db.get_value('Employee', { user_id: frappe.session.user }, ['name', 'employee_name', 'department'])
                .then(r => {
                    let values = r.message;
                    if (values) {
                        frm.set_value('custom_mr_raised_by', values.name);
                        frm.set_value('custom_department', values.department);
                        // Display employee name instead of Employee ID in the link field
                        frm.fields_dict['custom_mr_raised_by'].set_formatted_input(values.employee_name);
                    }
                });
        }
    },
    custom_mr_raised_by: function (frm) {
        // When the field is changed manually, show the employee name as display
        if (frm.doc.custom_mr_raised_by) {
            frappe.db.get_value('Employee', frm.doc.custom_mr_raised_by, 'employee_name')
                .then(r => {
                    if (r.message && r.message.employee_name) {
                        frm.fields_dict['custom_mr_raised_by'].set_formatted_input(r.message.employee_name);
                    }
                });
        }
    },
